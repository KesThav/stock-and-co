...
### Response time over time by threadsGroup
...
ggplotly(df_jm %>% 
           ...
           mutate(interval = cut(difference,seq(0, max(difference)+10, 60),dig.lab=10)) %>%
           ungroup() %>%
           mutate(interval_threads = cut(grpThreads,seq(0,max(grpThreads) + 10,1000),dig.lab=10)) %>%
           ...
           tidyr::separate(data = .,col = type,sep = "_",into = c("func","type"),remove = FALSE) %>%
           ggplot(.)  + geom_line(aes(x=lower_bound,y=response,color=type)) + facet_grid(func~lower_bound_threads) + theme_bw(),width = 1000,height=500)
```

Row
----------------------

### Distribution of response time per sentBytes, function and type
...
a = df_jm %>%
  #filter(success == "success") %>%
  group_by(type,grpThreads) %>%
  summarise(mean = mean(sentBytes)) %>%
  ungroup() %>% 
  tidyr::separate(data = .,col = type,sep = "_",into = c("func","type"),remove = FALSE) %>%
  mutate(func = ifelse(func == "createProduct", "addProduct",
                       ifelse(func == "productBoughtByUser", "products-users",func))) 

ggplotly(ggplot(a,aes(x=grpThreads,y=mean,color=type)) +
  geom_point() +
  facet_grid(.~func) + theme_bw()+ ylab("Response time (second)"),width = 1000,height=500) 
