...
files = c("few_data/sequential_createProduct_GraphQL",
          ...
          "lotof_data/sequential_createProduct_GraphQL",
          "lotof_data/sequential_createProduct_REST",
          "lotof_data/sequential_userThatBoughtProduct_GraphQL",
          "lotof_data/sequential_userThatBoughtProduct_REST",
          "lotof_data/sequential_getUsers_REST",
          "lotof_data/sequential_getUsers_GraphQL")
...
df_s = tmp_df %>% 
  left_join(df_s %>%
              ...
              ungroup() ,by=c("category","func","type","requests")) %>%
...
plot_df_s = df_s %>% filter(func != "userThatBoughtProduct")

p1 = ggplot(plot_df_s,aes(x=requests,y=mean_time,color=type)) + geom_line()  + facet_wrap(category~func,scales="free_y") + theme_bw()   
...
ggplotly(ggplot(a,aes(x=Threads,y=mean/1000,color=type)) +
  geom_point() +
  facet_grid(.~func) + theme_bw()+ ylab("Response time (second)")) 
...
ggplotly(df_jm %>% 
           ...
           summarise(response = mean(elapsed/100,na.rm=T),n=n()) %>%
           ungroup() %>%
           mutate(lower_bound = str_match(as.character(interval),"(?<=\\()(.*)(?=,)")[,2]) %>%
           filter(!is.na(lower_bound)) %>% 
           mutate(lower_bound = as.integer(lower_bound)) %>%
           tidyr::separate(data = .,col = type,sep = "_",into = c("func","type"),remove = FALSE) %>%
           ggplot(.)  + geom_line(aes(x=lower_bound,y=response,color=type)) + facet_wrap(func~.) + theme_bw())
...
ggplotly(df_jm %>% 
           ...
           mutate(interval = cut(difference,seq(0, max(difference)+10, 60),dig.lab=10)) %>%
           ungroup() %>%
           mutate(interval_threads = cut(grpThreads,seq(0,max(grpThreads) + 10,2000),dig.lab=10)) %>%
           group_by(type,interval,interval_threads) %>% 
           summarise(response = mean(elapsed/100,na.rm=T),n=n()) %>%
           ungroup() %>%
           mutate(lower_bound = str_match(as.character(interval),"(?<=\\()(.*)(?=,)")[,2]) %>%
           ...
           tidyr::separate(data = .,col = type,sep = "_",into = c("func","type"),remove = FALSE) %>%
           ggplot(.)  + geom_line(aes(x=lower_bound,y=response,color=type)) + facet_grid(func~lower_bound_threads) + theme_bw())
```
